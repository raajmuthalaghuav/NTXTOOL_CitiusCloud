#!/usr/bin/env python3
##############################################################################
# SCRIPT NUTANIX TOOLS FOR AHV v1.7.9                                        #
# PYTHON 3.6                                                                 #
...
##HOW TO RUN
## WITH CENTRAL INFO
## python NUTANIX_TOOLS_FOR_AHV_v1.7.9_WEB_VERSION_NO_INPUT.py 10.26.1.2 admin Pass1010., CENTRAL 10.26.1.147 admin Pass1010.,
## WITHOUT CENTRAL
## python NUTANIX_TOOLS_FOR_AHV_v1.7.9_WEB_VERSION_NO_INPUT.py 10.26.1.2 admin Pass1010., NOCENTRAL
...
version_soft = 'Nutanix tools for AHV v1.7.9'
print(version_soft)
start = time.time()
...
try:
    ...
    pe_resilency = requests.get(url + '/PrismGateway/services/rest/v1/cluster/domain_fault_tolerance_status',
                                auth=(username, password), verify=False, timeout=TIMEOUT)
    pe_storage_pool = requests.get(url + '/PrismGateway/services/rest/v1/storage_pools/',
                                auth=(username, password), verify=False, timeout=TIMEOUT)
    pe_ahvdisk = requests.get(url + '/PrismGateway/services/rest/v2.0/disks/', auth=(username, password), verify=False,
                              timeout=TIMEOUT)
    pe_authconfig = requests.get(url + '/PrismGateway/services/rest/v2.0/authconfig/', auth=(username, password), verify=False,
                              timeout=TIMEOUT)
...
if (pe_vm_info.status_code == requests.codes.ok
        ...
        and pe_storage_pool.status_code == requests.codes.ok
        and pe_authconfig.status_code == requests.codes.ok):
...
try:
    confirmpc = sys.argv[4]
    if confirmpc == "CENTRAL":
        confirm = 'y'
        print('LOGIN PRISM CENTRAL')
        pc_ip = sys.argv[5]
        pc_url = ('https://' + pc_ip + ':9440')
        pc_username = sys.argv[6]
        pc_password = sys.argv[7]
        ...
        pcimage_raw = pc_image.json()
        pcimage_total= pcimage_raw['metadata']['total_matches']
...
if pe_success_login == 100:
    ...
    ##STORAGEPOOL
    if pe_storage_pool.status_code == requests.codes.ok:
        out_json = pe_storage_pool.json()
        storagepool_total =str(out_json['metadata']['totalEntities'])
        storagepool = out_json['entities']

    ##AUTHCONFIG
    if pe_authconfig.status_code == requests.codes.ok:
        out_json = pe_authconfig.json()

        if 'directory_list' in out_json.keys(): 
            if out_json['directory_list']:
                direcoryahv = []
                for x in out_json['directory_list']:
                    if 'directory_type' in x.keys():
                        directorytype = x['directory_type']
                        directorycontype = x['connection_type']
                        directoryurl = x['directory_url']
                        directorydomain = x['domain']
                        directoryname = x['name']
                        directorysearch = x['group_search_type']
                        directoryaccount = x['service_account_username']
                        direcoryahv.append([directoryname,[directorytype,directorycontype,directoryurl,directorydomain,directorysearch,directoryaccount]])
            else:
                direcoryahv = None
    ...
    if confirm == 'y':
        workbook = xlsxwriter.Workbook(
            directory + outfilename + '_VM_INFO_PE_PC' + '(' + cluster_name + ')' + '_DATE_' + '(' + date_time + 'HRS' + ')' + '.xlsx')
    else:
        workbook = xlsxwriter.Workbook(
            directory + outfilename + '_VM_INFO_PE_' + '(' + cluster_name + ')' + '_DATE_' + '(' + date_time + 'HRS' + ')' + '.xlsx')
    ...
    worksheet11 = workbook.add_worksheet('STORAGE POOL')
    ...
    if confirm == 'y':
        worksheet9.set_column('A:E', width)
    worksheet10.set_column('A:T', width)
    worksheet11.set_column('A:T', 35)
    ...
    worksheet0.write('A11', 'TOTAL DISK',bold_0)
    worksheet0.write('A12', 'CLUSTER IP',bold_0)
    worksheet0.write('A13', 'CLUSTER DATA SERVICE IP',bold_0)
    worksheet0.write('A14', 'CLUSTER REDUNDANCY FACTOR',bold_0)
    worksheet0.write('A15', 'TOTAL VM',bold_0)
    worksheet0.write('A16', 'HA ENABLED',bold_0)
    worksheet0.write('A17', 'HA RESERVATION',bold_0)
    worksheet0.write('A18', 'HA HOST TOLERATE',bold_0)
    worksheet0.write('A19', 'HA STATE',bold_0)
    worksheet0.write('A20', 'RESILIENCY STATUS LEVEL',bold_0)
    worksheet0.write('A21', 'N°DISK CAN BE FAIL ON METADATA',bold_0)
    worksheet0.write('A22', 'DISK IS REBUILDING ',bold_0)
    worksheet0.write('A23', 'N°DISK CAN BE FAIL ON ERASURE_CODE_STRIP_SIZE',bold_0)
    worksheet0.write('A24', 'DISK IS REBUILDING ',bold_0)
    worksheet0.write('A25', 'N°DISK CAN BE FAIL ON EXTENT_GROUPS',bold_0)
    worksheet0.write('A26', 'DISK IS REBUILDING ',bold_0)
    worksheet0.write('A27', 'N°DISK CAN BE FAIL ON OPLOG',bold_0)
    worksheet0.write('A28', 'DISK IS REBUILDING ',bold_0)
    worksheet0.write('A29', 'DIRECTORY NAME ',bold_0)
    worksheet0.write('A30', 'DIRECTORY TYPE ',bold_0)
    worksheet0.write('A31', 'DIRECTORY CONNECTION ',bold_0)
    worksheet0.write('A32', 'DIRECTORY URL ',bold_0)
    worksheet0.write('A33', 'DIRECTORY DOMAIN ',bold_0)
    worksheet0.write('A34', 'DIRECTORY USER ACCOUNT ',bold_0)
    worksheet0.write('A35', 'DIRECTORY SEARCH TYPE ',bold_0)
    worksheet0.write('A36', 'PRISM CENTRAL URL',bold_0)
    worksheet0.write('A37', 'DATE',bold_0)
    worksheet0.write('A38', 'VERSION SCRIPT TOOLS',bold_0)
    ...
    worksheet7.write('E1', 'STORAGECONTENT UUID', bold)
    worksheet7.write('F1', 'STORAGECONTENT NAME', bold)
    worksheet7.write('G1', 'NDFS PATH', bold)
    ...
    if confirm == 'y':
        ...
        worksheet9.write('E1', 'Image Owner', bold)
    ...
    worksheet10.write('L1', 'DISK UUID', bold)
    worksheet10.write('M1', 'DISK HEALTH', bold)
    worksheet11.write('A1', 'POOL UUID', bold)
    worksheet11.write('B1', 'POOL NAME', bold)
    worksheet11.write('C1', 'POOL CAPACITY IN TB', bold)
    worksheet11.write('D1', 'POOL DISK UUID', bold)
    worksheet11.write('E1', 'DISK HOST OWNER', bold)
    worksheet11.write('F1', 'DISK TIER', bold)
    worksheet11.write('G1', 'DISK SIZE', bold)
    worksheet1.autofilter('D1:Z1')
    worksheet2.autofilter('A1:Z1')
    worksheet3.autofilter('A1:Z1')
    worksheet4.autofilter('A1:Z1')
    worksheet5.autofilter('A1:Z1')
    worksheet6.autofilter('A1:Z1')
    worksheet7.autofilter('A1:Z1')
    worksheet8.autofilter('A1:Z1')
    if confirm == 'y':
       worksheet9.autofilter('A1:Z1')
    worksheet10.autofilter('A1:Z1')
    worksheet11.autofilter('A1:Z1')
                            ...
                            if 'guest_tools' in project['spec']['resources'].keys():
                                if 'ngt_state' in project['spec']['resources']['guest_tools'][
                                    ...
                                    if lastvmuuid == vm[1]:
                                        merge1= row
                                        merge2 = row+1
                                        worksheet1.merge_range('V'+str(merge1)+':V'+str(merge2),'Enable for check',bold2)
                                        merge1 = 0
                                        merge2 = 0
    ...
    for scontainer in container:
        ...
        worksheet4.write(row, col + 2, (scontainer[7]['storage.usage_bytes']),bold2) ## MAX CAPACITY IN BYTES
        worksheet4.write(row, col + 3, form2,bold2) 
        worksheet4.write(row, col + 4, '='+str(scontainer[2])+'/'+(str(scontainer[3])),bold2)
    ...
    for image_size in image:
        if 'vm_disk_size' in image_size.keys():
            ...
            worksheet8.write(row, col + 2, image_size1/1024/1024/1024,bold2)
    ...
    col = 0
    if confirm == 'y':
        if pcimage_total < 500:
            imagelen= '{"kind": "image","offset": 0,"length": 500}'
            imagecall = session.post(pc_url + '/api/nutanix/v3/images/list', imagelen)
            imageraw = imagecall.json()            
            for x in imageraw['entities']:
                row = row + 1
                imagename = x['spec']['name']
                worksheet9.write(row, col, imagename,bold2)
                if 'image_type' in x['spec']['resources'].keys(): 
                    imagetype = x['spec']['resources']['image_type']
                    worksheet9.write(row, col + 1, imagetype,bold2)
                else:
                    worksheet9.write(row, col + 1, '-',bold2)
                if 'size_bytes' in x['status']['resources'].keys():
                    imagesize = x['status']['resources']['size_bytes']
                    worksheet9.write(row, col + 2, imagesize/1024/1024/1024,bold2)   
                else:
                    worksheet9.write(row, col + 2, '-',bold2)
                if 'creation_time' in x['metadata'].keys():
                    imagecreation = x['metadata']['creation_time']
                    worksheet9.write(row, col + 3, imagecreation,bold2)
                else:
                    worksheet9.write(row, col + 3, '-',bold2)
                if 'owner_reference' in x['metadata'].keys():
                    if 'name' in x['metadata']['owner_reference'].keys():
                        imageowner = x['metadata']['owner_reference']['name']
                        worksheet9.write(row, col + 4, imageowner,bold2)
                    else:
                        worksheet9.write(row, col + 4, '-',bold2)
                else:
                    worksheet9.write(row, col + 4, '-',bold2)
            ...
            offset_s = 0
            length_s = poolingvmapi
            while (offset_s <= pcimage_total):
                imagelen = '{"kind": "image","offset": ' + str(offset_s) + ',"length": ' + str(length_s) + '}'
                length_s = length_s + poolingvmapi
                offset_s = offset_s + poolingvmapi
                imagecall = session.post(pc_url + '/api/nutanix/v3/images/list', imagelen)
                imageraw = imagecall.json()
    ...
    for ahvdisk_raw in ahvdisk:
        ...
        diskuuidahv = ahvdisk_raw['disk_hardware_config']['disk_uuid']
        ...
        worksheet10.write(row, col + 11, diskuuidahv,bold2)
        worksheet10.write(row, col + 12, diskhealth,bold2)
        diskahvlist.append([diskuuidahv,[host,tier,disksize/1024/1024/1024]])
    ...
    while exit is 0:
        for storagepool_raw in storagepool:
            row = row + 1
            UUID = storagepool_raw['storagePoolUuid']
            NAME = storagepool_raw['name']
            CAPACITY = storagepool_raw['capacity']
            DISKUUID = storagepool_raw['diskUuids']
            POOLEN = len(DISKUUID)
            counter = storagepool_total
            if counter is not 0:
                worksheet11.merge_range('A2'+':A'+str(POOLEN+1),UUID, bold2)
                worksheet11.merge_range('B2'+':B'+str(POOLEN+1),NAME, bold2)
                worksheet11.merge_range('C2'+':C'+str(POOLEN+1),CAPACITY/1024/1024/1024/1024, bold2)
                counter = (int(storagepool_total)-1)
                for x in DISKUUID:
                    raw = raw + 1
                    worksheet11.write(raw, col + 3, x,bold2)
                    for y in diskahvlist:
                        if x == y[0]:
                           worksheet11.write(raw, col + 4, y[1][0],bold2)
                           worksheet11.write(raw, col + 5, y[1][1],bold2)
                           worksheet11.write(raw, col + 6, y[1][2],bold2)
                if counter == 0:
                    exit = 1                 
    ...
    worksheet0.write(row + 10, 1, POOLEN, boldcell)
    worksheet0.write(row + 11, 1, clusterip, boldcell)
    worksheet0.write(row + 12, 1, clusterdataip, boldcell)
    worksheet0.write(row + 13, 1, cluster_rf, boldcell)
    worksheet0.write(row + 14, 1, num_vm_cluster, boldcell)
    worksheet0.write(row + 15, 1, failover_en, boldcell)
    worksheet0.write(row + 16, 1, reservation, boldcell)
    worksheet0.write(row + 17, 1, host_tolerate, boldcell)
    worksheet0.write(row + 18, 1, ha_state, boldcell)
            ...
            if 'numberOfFailuresTolerable' in resilency_raw['componentFaultToleranceStatus']['METADATA'].keys():
                ...
                if 'underComputation' in resilency_raw['componentFaultToleranceStatus']['METADATA'].keys():
                    ...
                    if resilency_raw['domainType'] == 'DISK':
                        worksheet0.write(row + 19, 1, resilency_raw['domainType'], boldcell)
        ...
        if 'EXTENT_GROUPS' in resilency_raw['componentFaultToleranceStatus'].keys():
            if 'numberOfFailuresTolerable' in resilency_raw['componentFaultToleranceStatus']['EXTENT_GROUPS'].keys():
                ...
                if 'underComputation' in resilency_raw['componentFaultToleranceStatus']['EXTENT_GROUPS'].keys():
                    ...
                    if resilency_raw['domainType'] == 'DISK':
                        worksheet0.write(row + 24, 1, numberofdiskfail, boldcell)
                        worksheet0.write(row + 25, 1, disk_rebuild, boldcell)
        if 'OPLOG' in resilency_raw['componentFaultToleranceStatus'].keys():
            if 'numberOfFailuresTolerable' in resilency_raw['componentFaultToleranceStatus']['OPLOG'].keys():
                ...
                if 'underComputation' in resilency_raw['componentFaultToleranceStatus']['OPLOG'].keys():
                    ...
                    if resilency_raw['domainType'] == 'DISK':
                        worksheet0.write(row + 26, 1, numberofdiskfail, boldcell)
                        worksheet0.write(row + 27, 1, disk_rebuild, boldcell)
    if direcoryahv == None:
        worksheet0.write(row + 28, 1, 'No directory is configure', boldcell)
        worksheet0.write(row + 29, 1, '-', boldcell)
        worksheet0.write(row + 30, 1, '-', boldcell)
        worksheet0.write(row + 31, 1, '-', boldcell)
        worksheet0.write(row + 32, 1, '-', boldcell)
        worksheet0.write(row + 33, 1, '-', boldcell)
        worksheet0.write(row + 34, 1, '-', boldcell)

    else:
        worksheet0.write(row + 28, 1, direcoryahv[0][0], boldcell)
        worksheet0.write(row + 29, 1, direcoryahv[0][1][0], boldcell)
        worksheet0.write(row + 30, 1, direcoryahv[0][1][1], boldcell)
        worksheet0.write(row + 31, 1, direcoryahv[0][1][2], boldcell)
        worksheet0.write(row + 32, 1, direcoryahv[0][1][3], boldcell)
        worksheet0.write(row + 33, 1, direcoryahv[0][1][5], boldcell)
        worksheet0.write(row + 34, 1, direcoryahv[0][1][4], boldcell)

    if confirm == 'y':
        worksheet0.write(row + 35, 1, pc_url, boldcell)
    else:
    	worksheet0.write(row + 35, 1, 'No prism central info selected', boldcell)
    worksheet0.write(row + 36, 1, today.strftime('Running on %d, %b %Y'), boldcell)
    worksheet0.write(row + 37, 1, version_soft, boldcell)
    workbook.close()

    if confirm == 'y':
        namefile = '\n' + outfilename + '_VM_INFO_PE_PC' + '(' + cluster_name + ')' + '_DATE_' + '(' + date_time + 'HRS' + ')' + '.xlsx'
    else:
        namefile = '\n' + outfilename + '_VM_INFO_PE_' + '(' + cluster_name + ')' + '_DATE_' + '(' + date_time + 'HRS' + ')' + '.xlsx'
